...
// Dock button pixel width — used to keep panels from overlapping the dock
// md: icon only (~40px button + 12px left margin + 12px gap = 64px)
// lg: with label (~120px button + 12px left margin + 12px gap = 144px)
...
export function FlowPanelOverlay({ mainView, onMainViewChange, activePanel, onPanelChange }: Props) {
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-40">
      ...
      {/* ── Graph panel — always behind the dock ── */}
      <AnimatePresence>
        {mainView === 'graph' && (
          <motion.div
            key="graph-panel"
...
// Expose dock width constants for other components if needed
